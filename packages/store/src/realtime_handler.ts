...
export class RealtimeHandler<T extends SupabaseClient> {
    ...
    public add(
        ...
        if (this.channelFactories.has(channel.topic)) {
            this.logger.warn(`Overwriting existing channel factory for topic: ${channel.topic}`);
    ...
    public start() {
        if (this.started) {
            this.logger.warn('RealtimeHandler has already been started. Ignoring subsequent start call.');
            return () => { };
        }
    ...
    private async subscribeToChannel(channel: RealtimeChannel) {
        if (channel.state === 'joined' || channel.state === 'joining') {
            this.logger.debug(`Channel '${channel.topic}' is already joined or joining. Skipping subscribe.`);
            return;
        }
        ...
        switch (status) {
            ...
            case REALTIME_SUBSCRIBE_STATES.CHANNEL_ERROR: { // We'll just reconnect when the tab becomes visible again. // if the tab is hidden, we don't really care about reconnection
                if (err && isTokenExpiredError(err)) {
                    this.logger.error(`Token expired causing channel error in '${topic}'. Refreshing session.`);
                    this.resubscribeToChannel(topic);
                } else {
