...
import { programs, questions } from "./schema";
...
export type QnaStateChangeCallback = (program: string, oldState: boolean, newState: boolean, logger: Logger) => void;

export const onQnaStateChange = (supabase: SupabaseClient, callback: QnaStateChangeCallback, logger: Logger) => {
    return supabase
        ...
        .on<typeof programs.$inferSelect>(
            ...
            (payload) => {
                if (payload.old.open === undefined) {
                    logger.warn(`Unable to read new Q&A status for ${payload.new.program} (new state is open: ${payload.new.open}), exiting.`);
                    return;
                }
                logger.info(`Received Q&A state change. ${payload.new.program} state changed from ${payload.old.open} to ${payload.new.open}`)
                callback(payload.new.program, payload.old.open, payload.new.open, logger);
            }
        )
}
