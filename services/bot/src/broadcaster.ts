...
export const handleQnaStateChange = async (program: string, oldOpenState: boolean, newOpenState: boolean) => {
    const logger = (container.logger as PinoLoggerAdapter).child({
        label: "handleQnaStateChange",
    });
    const { ok, error, result: channel } = await getChannel(program);
    if (!ok) {
        logger.error({ error }, `An error occurred while fetching the channel for ${program}, exiting.`);
        return;
    }
    const embed = buildQnaStateEmbed(program, oldOpenState, newOpenState);
    const sent = await trycatch(broadcast(channel, [embed]));
    if (!sent.ok) {
        logger.error({ error }, `An error occurred while sending the state change for ${program}, exiting.`);
        return;
    }
    logger.info(`Successfully broadcasted state change from open: ${oldOpenState} to open: ${newOpenState} for ${program}.`);
}
...
export const startBroadcaster = (logger: Logger) => {
    ...
    realtime.add((supabase) => onQnaStateChange(supabase, handleQnaStateChange, logger))
